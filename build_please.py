...
FOLDER = "hope"  # <- align with your YAML paths
...
CONFIG = {
    ...
    # Output Filenames
    "panel_out": "features_and_dh_returns.parquet",
    "panel_outgreeks": "features_greeks_only.parquet",
    ...
    # Model Filters & Winsor
    "USE_FIXED_MID_FLOOR": True,
    "MID_PRICE_FIXED_FLOOR": 0.50,
    "MID_FLOOR_CLIP": (0.50, 10.00),
...
def to_num(s: pd.Series) -> pd.Series:
    """Shorthand for numeric conversion with coercion."""
    return pd.to_numeric(s, errors="coerce")

def safe_numeric_col(df: pd.DataFrame, col: str) -> pd.Series:
    """Return numeric Series; handles duplicates/missing cols."""
...
def safe_math_op(x: pd.Series, op_func) -> pd.Series:
    """Generic wrapper for safe math operations."""
...
def _y_distribution_diagnostics(y: pd.Series) -> Dict[str, float]:
    ...
    out = {
        "n": int(len(y)), "mean": float(y.mean()), "std": float(y.std(ddof=1)) if y.std(ddof=1) > 0 else np.nan,
    ...
    out.update({f"q{int(k*1000):03d}": float(v) for k,v in y.quantile([0.005,0.01,0.05,0.5,0.95,0.99,0.995]).to_dict().items()})
    return out
...
def _save_distribution_plots(y: pd.Series, hist_path: Path, qq_path: Path, pp_path: Path):
    ...
    plt.savefig(hist_path, dpi=140); plt.close(fig)
    
    # Q-Q
    fig = plt.figure()
    probplot(y, dist="norm", plot=plt); plt.title("Q–Q plot of y_price (modeling panel)")
    plt.savefig(qq_path, dpi=140); plt.close(fig)
    ...
    if np.isfinite(sd) and sd > 0:
        ...
        plt.xlabel("Normal CDF (theoretical)"); plt.ylabel("Empirical CDF"); plt.title("P–P plot")
...
def corwin_schultz_spread(df_hlc: pd.DataFrame, overnight_adjust: bool = True) -> pd.Series:
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        gamma = np.log(pd.concat([h, h.shift(1)], axis=1).max(axis=1) / 
                       pd.concat([l, l.shift(1)], axis=1).min(axis=1)).pow(2)
...
@dataclass
class B76Inputs:
    F: float; K: float; tau: float; r: float; is_call: bool

def _d1_d2(F, K, sigma, tau):
    if sigma <= 0 or tau <= 0 or F <= 0 or K <= 0: return np.nan, np.nan
...
def implied_vol_b76(inputs: B76Inputs, price_obs: float, lo=1e-6, hi=8.0) -> Optional[float]:
    if not (np.isfinite(price_obs) and price_obs > 0): return np.nan
    try:
...
def load_sofr() -> pd.DataFrame:
    if not CONFIG["use_sofr"]: return pd.DataFrame()
...
def build_futures_series_robust(opt: pd.DataFrame) -> pd.DataFrame:
    ft_opt = opt.groupby("date", as_index=False)["futuresettlementprice"].median().rename(columns={"futuresettlementprice": "F_t_opt"})
    ...
    F = F[["date", "F_t"]].sort_values("date").dropna(subset=["F_t"]).groupby("date", as_index=False).last()
    
    if F.empty: raise ValueError("No futures levels found.")
    
    full_idx = pd.date_range(min(F["date"]), max(F["date"]) + pd.Timedelta(days=1), freq="D").date
    F = F.set_index("date").sort_index()
    F = F[~F.index.duplicated(keep="last")].reindex(full_idx).ffill().reset_index().rename(columns={"index": "date"})
    F["F_tp1"] = F["F_t"].shift(-1)
    return F[["date", "F_t", "F_tp1"]]

# ------------------------------ COMPLEX FEATURES ------------------------------
def add_nextday_mid_by_signature(df: pd.DataFrame) -> pd.Series:
...
def build_rr_bf_25(df_rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for d in dates:
        ...
        # Calculate RR (Risk Reversal) and BF (Butterfly)
...
def build_daily_oi_wall_fallback(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not out_dates: return pd.DataFrame(columns=["date", "dist_to_wall_fallback", "gex_proxy_fallback"])
    ...
    return pd.DataFrame({"date": out_dates}).merge(fb_wall[["date","dist_to_wall_fallback"]], on="date", how="left") \
...
def apply_low_risk_missingness_fixes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"date", "dist_to_wall", "gex_proxy"}.issubset(df.columns):
        dlev = df.groupby("date", as_index=False).agg(dist=("dist_to_wall", "median"), gex=("gex_proxy", "median")).sort_values("date")
        df = df.merge(dlev.assign(ddist_to_wall_date=dlev["dist"].diff(), dgex_proxy_date=dlev["gex"].diff())[["date", "ddist_to_wall_date", "dgex_proxy_date"]], on="date", how="left")
    ...
    # Scale-free IV-change proxies
    if "d_iv_level_filled" in df.columns: df["ivchg_proxy"] = df["d_iv_level_filled"]
    elif "d_iv_level" in df.columns: df["ivchg_proxy"] = df["d_iv_level"]
    if "d_iv_atm" in df.columns: df["atm_ivchg_proxy"] = df["d_iv_atm"]
    ...
    for c in ["ddist_to_wall", "dgex_proxy", "oi_herf", "vega_ivchg_proxy"]:
        if c in df.columns: df[f"{c}_is_na"] = df[c].isna().astype(int)
    return df
...
def attach_futures_next_trading_day(opt_df, fut_df, date_col="date", f_col="F_t"):
    ...
    fut_cal[f"{f_col}_tp1"] = fut_cal[f_col].shift(-1)
...
def build_panel() -> pd.DataFrame:
    ...
    # --- FIX: Parse dates and Calculate DTE immediately ---
    ...
    # Rename vendor Greeks
    opt.rename(
        columns={c: f"{c}_src" for c in ["delta", "gamma", "vega", "theta", "rho"] if c in opt.columns},
        inplace=True,
    )
    opt.replace(CONFIG["dummy_sentinel"], np.nan, inplace=True)

    # Filter DTE and Volume
    mask = (opt["dte"] >= CONFIG["dte_min"]) & (opt["dte"] <= CONFIG["dte_max"]) & (safe_numeric_col(opt, "volume") > 0)
    opt = opt[mask].copy()
    n_after_dte = len(opt)

    # -----------------------------
    # KEY SPEED-UP: futures series
    # -----------------------------
    # Build a single robust futures series (F_t, F_tp1) once and reuse it everywhere.
    ...
    def construct_mid(row):
        ...
        if np.isfinite(b) and np.isfinite(o) and b > 0 and o >= b:
            return 0.5 * (b + o)
        for px in [row.get("settlementprice"), row.get("closeprice")]:
    ...
    # Model Greeks (unchanged logic; still row-wise for exact same behaviour)
    def greeks_row(row):
        inp = B76Inputs(
            F=row["F_t"],
            K=row["strike"],
        ...
        if not np.isfinite(sigma):
            return pd.Series(
                {k: np.nan for k in ["delta_model", "gamma_model", "vega_model", "theta_model", "rho_model"]}
            )
    ...
    df["mispricing_vega"] = np.where(
        np.isfinite(v) & (np.abs(v) > 1e-8), df["mispricing_raw"] / v, np.nan
    ).clip(-5, 5)
    df["mispricing_price"] = np.where(
        np.isfinite(m) & (m > 1e-8), df["mispricing_raw"] / m, np.nan
    ).clip(-1, 1)

    if "date" in df.columns:
        for c in ["mispricing_vega", "mispricing_price"]:
            df[f"{c}_xs"] = (
                df[c] - df.groupby("date")[c].transform("median")
            ).clip(lower=df[c].min(), upper=df[c].max())

    df["mispricing_price_bps"] = np.where(
        np.isfinite(m) & (m > 1e-10), (df["mispricing_raw"] / m) * 1e4, np.nan
    )
    ...
    def build_atm_iv_from_forward(rows):
        ...
        for (d, e), g in rows.groupby(["date", "expiration"], sort=False):
            ...
            if not (np.isfinite(F_med) and tau_md > 0): out.append((d, e, np.nan)); continue
            ...
            else: wts = pd.Series(1.0, index=near.index)
            ...
            if near.empty: out.append((d, e, np.nan)); continue
            
            # Interpolation Helper
            def interp_side(df_side):
                if df_side.empty: return np.nan
                x, y, w = df_side["strike"].values, df_side["mid_t"].values, wts.loc[df_side.index].values
                m = np.isfinite(x) & np.isfinite(y); x, y, w = x[m], y[m], w[m]
            ...
            for px, is_call in [(interp_side(near[near["is_call"]==1]), True), (interp_side(near[near["is_call"]==0]), False)]:
                if np.isfinite(px) and px > 0:
                    v = implied_vol_b76(B76Inputs(F_med, F_med, tau_md, r_md, is_call), px, CONFIG.get("iv_lower", 1e-6), CONFIG.get("iv_upper", 8.0))
                    if np.isfinite(v) and v > 0: vols.append(v)
    ...
    def local_smile(group):
        ...
        for bw in [0.10, 0.15, 0.20, CONFIG["atm_window_logmny"]]:
            ...
            if len(g) < max(3, CONFIG["min_points_local_fit"]): continue
            
            x, y = g["log_moneyness"].values, g["iv"].values
            if "opt_rel_spread_final" in g.columns:
                rs = to_num(g["opt_rel_spread_final"])
                mask = rs <= np.nanquantile(rs, 0.95)
                if mask.sum() >= 3: x, y = x[mask], y[mask]
                
            h = max(float(bw), 1e-6)
            w = np.maximum(0.0, 1.0 - (np.abs(x)/h)**3)**3
            
            try: c2, c1, c0 = np.polyfit(x, y, 2, w=w)
            except: c2, c1, c0 = np.polyfit(x, y, 2)
        ...
        if len(g) == 2:
            b1 = np.polyfit(g["log_moneyness"], g["iv"], 1)[0]
    ...
    for c in ["atm_iv", "smile_slope", "convexity_proxy"]: df[c] = np.nan
    ...
    # Smoothed ATM for standardization
    atm_date = df.groupby("date")["atm_iv"].median().rename("atm_iv_date_med").reset_index().sort_values("date")
    atm_date["atm_iv_smooth"] = atm_date["atm_iv_date_med"].ewm(halflife=CONFIG.get("ATM_SMOOTH_HALFLIFE", 5), min_periods=CONFIG.get("ATM_SMOOTH_MINP", 3)).mean().ffill().bfill()
    ...
    try:
        dif = (df.loc[df["log_moneyness"].abs()<=0.02, "iv"] - df.loc[df["log_moneyness"].abs()<=0.02, "atm_iv"]).abs()
        print(f"[ATM QA] |iv - atm_iv| median={np.nanmedian(dif):.4f}, p95={np.nanpercentile(dif,95):.4f}")
    except: pass

    # Refill NA IVs near ATM and recompute Greeks
    ...
    def _slope(g):
        ...
        if x.size < 2 or np.allclose(x, x[0]): return np.nan
    ...
    # Bucketed IV Lag
    df["mny_bin"] = pd.cut(df["log_moneyness"], bins=[-np.inf, -0.75, -0.35, -0.10, 0.10, 0.35, 0.75, np.inf], 
    ...
    med = df.groupby(["expiration", "is_call", "mny_bin", "price_bin", "date"], observed=True)["iv"].median().rename("iv_med").reset_index().sort_values("date")
...
# Futures RV & Skew
    # (use futures_core instead of rebuilding)
    ...
    df = df.merge(
        Ft[["date", "rv_pred", "rv_3d", "rv_5d", "rv_chg", "realskew_chg"]],
        on="date",
        how="left",
    )
    ...
    if not bitw.empty:
        ...
        # Beta adjustment – reuse futures_core instead of recomputing
        ...
        bitw["rs_bitw_btc_10"] = (
            (np.log(to_num(bitw[px_col])) - np.log(bitw["F_t"]))
            .diff()
            .shift(1)
            .rolling(10)
            .sum()
    ...
    # Microstructure (Futures) – unchanged, still uses Yahoo OHLC
    fy = load_standard_csv(CONFIG["paths"]["futures_yahoo"])
    if not fy.empty:
        fy.columns = [c.strip().lower() for c in fy.columns]
        if {"date", "high", "low", "close"}.issubset(fy.columns):
            ...
            cs = corwin_schultz_spread(
                fy.sort_values("date").drop_duplicates("date", keep="last").set_index("date")
            )
    ...
    df["opt_rel_spread_raw"] = np.where((df["bid"]>0) & (df["offer"]>0), (df["offer"]-df["bid"])/((df["offer"]+df["bid"])*0.5), np.nan)
    ...
    med_exp = df.groupby(["dte","is_call","mny_bin","price_bin"], observed=True)["opt_rel_spread_raw"].apply(lambda s: s.expanding().median().shift(1)).reset_index(drop=True)
    ...
    # Contract Extras
    df = df.sort_values([idcol, "date"])
    if "openinterest" in df.columns: df["d_openinterest"] = df.groupby(idcol)["openinterest"].diff()
    if {"volume","openinterest"}.issubset(df.columns): df["volume_to_oi"] = df["volume"] / (to_num(df["openinterest"]) + 1e-9)

    # Wall/GEX
    def wall_gex_calc(g):
        s = g.groupby("strike")["oi_eff"].sum().sort_values(ascending=False)
        if s.empty: return pd.Series([np.nan, np.nan, np.nan], index=["oi_herf", "dist_to_wall", "gex_proxy"])
        ...
        F = g["F_t"].median()
        dist = np.log(wall / F) if np.isfinite(wall) and F > 0 else np.nan
    ...
    for c in ["gex_proxy", "dist_to_wall"]: wg[f"d{c}"] = wg.groupby("expiration")[c].diff()
    ...
    # Sentiment Processing Helper
    def process_sentiment(file_key, val_col=None, cols_to_z=None, date_col="date"):
        raw = load_standard_csv(CONFIG["paths"][file_key])
        if raw.empty: return pd.DataFrame()
        raw.columns = [c.strip().lower() for c in raw.columns]
        dcol = "datetime" if "datetime" in raw.columns else date_col
        if dcol not in raw.columns: return pd.DataFrame()
        raw["date"] = pd.to_datetime(raw[dcol]).dt.date
        
        agg = raw.sort_values("date")
        if val_col: # Google Trends case
            agg = agg.rename(columns={val_col: "gt_level"}).groupby("date")["gt_level"].mean().reset_index().sort_values("date")
            agg["d_gt"] = agg["gt_level"].diff()
            agg["z_gt"] = zscore_series(agg["gt_level"], CONFIG["zscore_method"])
            return agg[["date", "z_gt"]]
        
        # Reddit/Reuters case
        out_cols = ["date"]
        for c, alias in cols_to_z.items():
            base = alias or f"{file_key}_{c}"
            agg[base] = to_num(agg.get(c, np.nan))
            agg[f"{base}_z"] = zscore_series(agg[base], CONFIG["zscore_method"])
            out_cols.append(f"{base}_z")
            
        tot_col = f"{file_key}_total"
        agg[tot_col] = agg[[k if v is None else v for k,v in cols_to_z.items()]].sum(axis=1)
        agg[f"{tot_col}_z"] = zscore_series(agg[tot_col], CONFIG["zscore_method"])
        out_cols.append(f"{tot_col}_z")
        return agg[out_cols]

    # Google Trends
    df = df.merge(process_sentiment("google_trends", val_col="value"), on="date", how="left")
    # Reuters
    df = df.merge(process_sentiment("rtrs", cols_to_z={"pos": "rtrs_pos", "neg": "rtrs_neg", "neu": "rtrs_neu"}), on="date", how="left")
    # Reddit
    df = df.merge(process_sentiment("reddit", cols_to_z={"positive": "reddit_pos", "neutral": "reddit_neu", "negative": "reddit_neg"}), on="date", how="left")

    # CryptoQuant derivs/on-chain (daily)
    ...
    if not cq.empty:
        ...
        if d_col in cq.columns:
            ...
            for col, ops in transforms:
                ...
                for op in ops:
                    new_name = f"{op}_{col}"
                    if op == "log": agg[new_name] = safe_log(base)
                    elif op == "log1p": agg[new_name] = safe_log1p(base)
                    elif op == "logit": agg[new_name] = logit(base)
                    elif op == "asinh": agg[new_name] = safe_asinh(base)
    ...
    # Next Day Valuation
    df = attach_futures_next_trading_day(
        df,
        futures_core.rename(columns={"F_t": "F_t"}),
        date_col="date",
        f_col="F_t",
    ).rename(columns={"F_t_tp1": "F_tp1"})
    df = df.sort_values([idcol, "date"])
    df["mid_tp1"] = df.groupby(idcol)["mid_t"].shift(-1)
    df["dte_tp1"] = df.groupby(idcol)["dte"].shift(-1)
    df["mid_tp1_combined"] = df["mid_tp1"].combine_first(add_nextday_mid_by_signature(df))
    ...
    if "date" in df.columns: df["y_price_xs"] = df["y_price"] - df.groupby("date")["y_price"].transform("mean")
    ...
    for col in ["theta_model", "gamma_model"]: 
        ...
        # Use gamma_scaled fallback for gamma
    ...
    if "date" in df.columns: df["price_per_vega_xs"] = df["price_per_vega"] - df.groupby("date")["price_per_vega"].transform("median")
    ...
    def add_futures_momenta(df, ks=(5, 10, 21),
                        date_col="date",
                        futures_col="F_t"):
        g = (
            df[[date_col, futures_col]]
            .drop_duplicates()
        ...
        for k in ks:
            col = f"futmom_{k}"
            g[col] = g[futures_col] / g[futures_col].shift(k) - 1.0  # simple k-day return
        return df.merge(
            g[[date_col] + [f"futmom_{k}" for k in ks]],
    ...
    outdir = CONFIG["outputs_dir"]; diagdir = CONFIG["diagnostics_dir"]
    outdir.mkdir(parents=True, exist_ok=True); diagdir.mkdir(parents=True, exist_ok=True)
    ...
    cols += ["z_gt", "rtrs_pos_z", "rtrs_neg_z", "rtrs_neu_z", "rtrs_total_z", "reddit_pos_z", "reddit_neg_z", "reddit_neu_z", "reddit_total_z"]
    cols += [c for c in df.columns if any(x in c for x in ["funding_rates", "leverage_ratio", "taker_buy", "liquidations", "spot", "der", "addresses_count"])] # Catch-all for CQ
    cols += [f"{c}_x_{m}" for c in interact_cols for m in ["mny", "tau"]]
    cols += ["bitw_mom_5_1", "bitw_mom_10_1", "bitw_mom_21_1", "bitw_mom_exbtc_10_1", "bitw_vol_10", "rs_bitw_btc_10"]
    ...
    cols += [f"d_{g}_{s}" for g in ["delta", "gamma", "vega", "theta", "rho"] for s in ["src", "model"]]
    ...
    # Diagnostics Output
    pd.DataFrame({
        "row_id": df["row_id"], "date": df["date"], "optionid": df.get("optionid", np.nan),
        "has_mid_t": has_mid_t.astype(int), "has_iv": has_iv.astype(int), "has_greeks": has_greeks.astype(int),
        "has_F_tp1": has_F_tp1.astype(int), "has_V_tp1": has_V_tp1.astype(int), "has_y_price": has_y_price.astype(int)
    }).to_csv(outdir / "availability_flags.csv", index=False)

    summary = pd.DataFrame({
        "stage": ["raw_options_loaded", "after_dte_filter_1_3", "post_merge_panel_rows", "has_mid_t", "has_iv", "has_greeks", "has_F_tp1", "has_V_tp1", "has_y_price"],
        "count": [n_raw_opt, n_after_dte, len(df), has_mid_t.sum(), has_iv.sum(), has_greeks.sum(), has_F_tp1.sum(), has_V_tp1.sum(), has_y_price.sum()]
    })
    ...
    # Save
    df_greeks = df_out[df_out[["delta_model","vega_model","gamma_model","theta_model"]].apply(_finite).all(axis=1)].copy()
    df_out.to_parquet(outdir / CONFIG["panel_out"], index=False)
    df_greeks.to_parquet(outdir / CONFIG["panel_outgreeks"], index=False)
    ...
    # HTML Report
    if CONFIG["make_html_report"]:
        miss = df_out.isna().sum().to_frame("n").assign(pct=lambda x: x["n"]/len(df_out)*100).sort_values("pct", ascending=False)
        miss.to_csv(outdir / "missingness_overview.csv")
        
        # Missingness by date
        cols_na = df_out.columns[df_out.isna().any()].tolist()
        if cols_na:
            df_out.groupby("date")[cols_na].apply(lambda g: g.isna().sum()).to_csv(outdir / "missingness_by_date_cols.csv")
            
        html = f"""<html><body>
        <h1>BTC Options Report</h1>
        <p>Rows: {len(df_out)}</p>
        <h3>Top Missing</h3>{miss.head(20).to_html()}
        <h2>Diagnostics</h2>
        <img src="diagnostics/y_price_hist_modeling.png" width="500"><br>
        <img src="diagnostics/y_price_qq_modeling.png" width="500">
        </body></html>"""
        with open(outdir / CONFIG["report_html"], "w", encoding="utf-8") as f: f.write(html)
