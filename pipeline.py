...
# Logical Bali-style build-up:
# I  → I+B  → I+B+S  → I+B+S+C  → I+B+S+C+N  → I+B+S+C+N+INTERACTIONS
# GROUP_LADDER = [
#     ["I"],
#     ["I", "B"],
#     ["I", "B", "S"],
#     ["I", "B", "S", "C"],
#     # ["I", "B", "S", "C", "N"],
#     # ["I", "B", "S", "C", "N", "INTERACTIONS"],
# ]

# def _validate_groups(ladder, universe):
#     for step in ladder:
#         for g in step:
#             if g not in universe:
#                 raise ValueError(f"Unknown group in ladder: {g}")
...
RUNS = [
    ...
    # RunSpec(base_config_path="./config.yaml", target="y_price",
    #         data_path=r"C:\Users\kawah\Documents\bitcoining\cleaned_input\spec2_nom1_mny1_w10.csv",
    #         groups=GROUPS, contracts_type="put", contracts_mny="all",)
]

# # 1) Pooled (all contracts, all moneyness) across the ladder
# for gs in GROUP_LADDER:
#     RUNS.append(
#         RunSpec(
...
# # 2) By side (call / put), all moneyness, across the ladder
# for side in [s for s in SIDES if s != "all"]:
#     for gs in GROUP_LADDER:
#         RUNS.append(
#             RunSpec(
#                 base_config_path=CONFIG_PATH,
...
# # 3) By moneyness (atm/otm/itm), pooled sides, across the ladder
# for mny in [m for m in MNY_BUCKETS if m != "all"]:
#     for gs in GROUP_LADDER:
#         RUNS.append(
#             RunSpec(
#                 base_config_path=CONFIG_PATH,
...
def time_split(
    ...
    """Split by unique calendar dates with an optional embargo."""
    d = pd.to_datetime(df[time_col])
    u = pd.Index(d.sort_values().unique())
    n = len(u)
    if n < 5:
    ...
    te_start = va_end + 1
    if purge_gap > 0:
        tr_end = max(-1, tr_end - purge_gap)
        va_end = max(va_start - 1, va_end - purge_gap)
    tr_dates = u[: tr_end + 1]
    va_dates = u[va_start : va_end + 1]
    te_dates = u[te_start:]
    ...
    def fit(self, y_train_model_space: np.ndarray) -> None:
        ...
        if self.enabled_transform:
            if self.kind == "asinh":
                ...
                LOG.info("[guardrails] transform=yeo_johnson | lambda_=%.4f", float(self._yj.lambdas_[0]))
            else:
...
def r2_os_xs(y_true: np.ndarray, y_pred: np.ndarray, ids: np.ndarray) -> float:
    ...
    num = np.mean((df["y_cs"] - df["yhat_cs"]) ** 2)
    den = np.mean(df["y_cs"] ** 2)
    return 1.0 - num / den
...
def clark_west_t(
    ...
    """
    Bali-style CW/DM test vs the zero benchmark:
    c_t = avg_i[ e0^2 - e1^2 ], where e0 = y (zero forecast),
    e1 = y - yhat (model forecast). If groups is not None, we
    average cross-sectionally within each group (day) and then
    run NW on the time series of c_t.
    """
    y = np.asarray(y_true, dtype=float)
    ya = np.asarray(y_pred_alt, dtype=float)

    # we only support zero benchmark here
    if y_pred_base is not None:
        raise ValueError("clark_west_t currently implemented only for zero benchmark.")

    e0 = y              # error of zero forecast
    e1 = y - ya         # error of model
    d_row = e0**2 - e1**2   # difference in squared errors

    if groups is None:
        d = d_row
    else:
        gser = pd.Series(groups)
        d = pd.Series(d_row).groupby(gser).mean().values  # cross-sectional avg per group
...
def clark_west_t_xs(
    ...
    """
    Cross-sectional CW test vs zero benchmark, in the spirit of Bali et al.
    We first demean y and yhat within each group (day), then build
    c_t = avg_i[ e0_cs^2 - e1_cs^2 ], and run an NW t-test on {c_t}.
    """
    if groups is None:
    ...
    # cross-sectional demeaning within each day
    df["y_cs"]  = df["y"]  - df.groupby("g")["y"].transform("mean")
    df["ya_cs"] = df["ya"] - df.groupby("g")["ya"].transform("mean")

    e0 = df["y_cs"]            # zero-forecast error in xs space
    e1 = df["y_cs"] - df["ya_cs"]
...
def _equal_day_weights(dates_arr):
    if dates_arr is None:
        return None
    s = pd.Series(dates_arr)
    cnt = s.map(s.value_counts())
    with np.errstate(divide="ignore", invalid="ignore"):
        w = 1.0 / cnt.to_numpy(dtype=float)
    w = w / np.nanmean(w) if np.nanmean(w) not in (0, np.nan) else w
    return w
                ...
                if is_lgbm:
                    ...
                    try:
                        ...
                        fit_kwargs["callbacks"] = [
                            lgb.early_stopping(stopping_rounds=32, first_metric_only=True),
...
def try_export_shap(
    ...
    max_display = int(sconf.get("max_display", 9))  # 9 for thesis-style top features
...
def build_predictions_frame(
    ...
    *,
    ...
    """
    Assemble predictions aligned 1-for-1 with filtered test rows and
    append 'test-slice columns' needed for later portfolio work.
    """
...
def run_variant(cfg: Dict[str, Any], variant_label: str, feat_cols: List[str]) -> None:
    ...
    fh_variant = add_file_logger(os.path.join(out_dir, "run.log"), level=cfg.get("logging", {}).get("level", "INFO"))
    ...
    try:
        ...
        elif ("price" in tname) or ("ret" in tname):
            target_kind = "per_dollar"
        else:
            target_kind = "per_dollar"  # safe default
        ...
        # Replace infinities (e.g., log ratios) with NaN before preprocessing to avoid sklearn errors
        ...
        summary.update(rows_loaded=int(len(df_raw)), rows_after_filters=int(len(df)))
        ...
        df_tr, df_va, df_te = time_split(
            ...
            cfg["split"]["val_frac"],
            purge_gap=int(cfg["split"].get("purge_gap", 1)),
        )

        # -------------------------------
        # Feature routing by model family
        # -------------------------------
        inter_cols = set((cfg.get("data", {}).get("feature_groups") or {}).get("INTERACTIONS", []))
        feat_cols_lin = list(feat_cols)  # linear + FFN (keep interactions)
        feat_cols_non = [c for c in feat_cols if c not in inter_cols]  # trees (drop interactions)
        if not feat_cols_non:
            raise ValueError("After dropping INTERACTIONS, no features left for tree models.")
        LOG.info(
            "[features] linear+FFN=%d (with interactions) | trees=%d (no interactions)",
            len(feat_cols_lin),
            len(feat_cols_non),
        )
        ...
        for part, d in zip(["train", "val", "test"], [df_tr, df_va, df_te]):
            missing = [c for c in (needed_feats + [target_col, raw_return_col]) if c not in d.columns]
            if missing:
                raise KeyError(f"Missing in {part}: {missing}")
        ...
        d_te = pd.to_datetime(df_te[date_col].to_numpy())

        def _q(a):
            qs = [0.0, 0.5, 0.9, 0.95, 0.99, 0.995, 0.999, 1.0]
            return {
                k: float(v)
                for k, v in zip(
                    ["min", "p50", "p90", "p95", "p99", "p99.5", "p99.9", "max"], np.nanquantile(a, qs)
                )
            }

        LOG.info("[target] train quantiles: %s", _q(y_tr_target))
        LOG.info("[target]   val quantiles: %s", _q(y_va_target))
        LOG.info("[target]  test quantiles: %s", _q(y_te_target_all))

        def _dispersion(y, dates):
            y = np.asarray(y, dtype=float)
            var_all = float(np.nanvar(y))
            std_all = float(np.nanstd(y))
            d = pd.to_datetime(dates)
            g = pd.Series(y).groupby(pd.Series(d).dt.normalize())
            var_day_mean = float(g.var(ddof=0).mean())
            sizes = g.size()
            return var_all, std_all, var_day_mean, float(sizes.mean()), int(sizes.min()), int(sizes.max())

        for nm, y, d in [("train", y_tr_target, d_tr), ("val", y_va_target, d_va), ("test", y_te_target_all, d_te)]:
            v, s, vdw, n_mean, n_min, n_max = _dispersion(y, d)
            LOG.info(
                "[target] %s: var=%.6g | std=%.6g | day-avg var=%.6g | obs/day mean=%.1f min=%d max=%d",
                nm,
                v,
                s,
                vdw,
        ...
        if guard._lo is not None and guard._hi is not None:
            z_train = np.arcsinh(y_tr_model / guard._s) if guard.kind == "asinh" else y_tr_model
            clipped_lo = np.mean(z_train < guard._lo)
            clipped_hi = np.mean(z_train > guard._hi)
            LOG.info(
                "[winsor diag] lower_q=%.4g upper_q=%.4g | scale=%.5g | clipped_lo=%.4f%% clipped_hi=%.4f%%",
                guard.lower_q,
                guard.upper_q,
                guard._s,
                100 * clipped_lo,
        ...
        # -------------------------------
        # Preprocessing (family-specific)
        # -------------------------------
        pp_cfg = cfg.get("preprocess", {}) or {}

        # Linear: as per YAML (may use YJ/scale if you set them there)
        pp_linear = preprocess_fit(
            X_tr_lin_raw,
            pp_cfg.get("linear_nn", {"variance_threshold": 0.0, "yeo_johnson": False, "standardize": False}),
        )

        # FFN: avoid double-transform (FFN pipeline already has Impute→YJ→Scale)
        pp_ffn = preprocess_fit(
            X_tr_lin_raw,
            {**pp_cfg.get("linear_nn", {}), "yeo_johnson": False, "standardize": False},
        )

        # Trees: typically no YJ/scale
        pp_trees = preprocess_fit(
            X_tr_non_raw,
            pp_cfg.get("trees", {"variance_threshold": 0.0, "yeo_johnson": False, "standardize": False}),
        )
        ...
        # -------------------------------
        # Filter non-finite y once; align
        # -------------------------------
        ...
        # Align other designs to the same rows
        ...
        df_te_small = df_te[[c for c in [tcol, mcol] if c in df_te.columns]].iloc[mask_te.nonzero()[0]].copy()

        # Winsor-aligned target (for metrics)
        g_te = g_te_all[mask_te]
        y_te_target_winsor = winsor_aligned_target(y_te_target, g_te, guard)

        # ------------------------------------------------
        # Build registry and model lists (by kind field)
        # ------------------------------------------------
        ...
        # ------------------------------------
        # CV / training per family & features
        # ------------------------------------
        tcfg = cfg.get("tuning", {})
        ...
        cv_cfg = (tcfg.get("cv") or {"n_splits": 5, "gap": 0, "use_train_plus_val": True})
        cv_records: List[Dict[str, Any]] = []

        LOG.info("Training linear (with interactions): %s | preprocess=%s", linear_list, pp_cfg.get("linear_nn", {}))
        models_lin = fit_family(
            ...
            d_va,
            cv_cfg,
            inverse_to_target_fn=_inverse_to_target,
            y_cv_target=y_cv_target,
        )

        LOG.info("Training FFN (with interactions; internal YJ+scale): %s", nn_list)
        ...
        LOG.info("Training trees (no interactions): %s | preprocess=%s", trees_list, pp_cfg.get("trees", {}))
        ...
        # (No NN-En / N-En here; N-En is built later in prediction space.)
        ...
        for name, m in models_lin.items():
            ...
            y_model_hat = guard.inverse(z_hat)
            y_hat = TargetNormalizer.invert(y_model_hat, g_te_all[mask_te])
        ...
        # ----------------------------------------------------
        # Single nonlinear ensemble in prediction space (N-En)
        # ----------------------------------------------------
        # Prediction ensemble uses RF / GBDT / DART / FFN.
        # SHAP for N-En is computed separately in export_shap_n_en.
        ...
        if nonlin_member_names_pred:
            nl_stack = np.column_stack(
                [preds_in_target_units[nm] for nm in nonlin_member_names_pred]
            )
            preds_in_target_units["N-En"] = nl_stack.mean(axis=1)

        # For SHAP, build a tree-only N-En ensemble from models_trees.
        # Drop FFN-based members and require presence in models_trees.
        nonlin_member_names_shap = [
            nm for nm in nonlin_member_names_pred
            if nm.split("#", 1)[0] != "ffn" and nm in models_trees
        ]

        if nonlin_member_names_shap:
            models_trees["N-En"] = build_equal_weight_ensemble(nonlin_member_names_shap, models_trees)
            if set(nonlin_member_names_shap) != set(
                [nm for nm in nonlin_member_names_pred if nm.split("#", 1)[0] != "ffn"]
            ):
                LOG.info(
                    "N-En SHAP built on tree members %s; prediction ensemble uses %s (incl. FFN).",
                    sorted(nonlin_member_names_shap),
                    sorted(nonlin_member_names_pred),
                )

        def _stats(a):
            return dict(min=float(np.nanmin(a)), med=float(np.nanmedian(a)), max=float(np.nanmax(a)))

        LOG.info("y_te_target stats: %s | y_te_raw stats: %s", _stats(y_te_target), _stats(y_te_raw))
        for nm, arr in list(preds_in_target_units.items()):
            denom = np.nanstd(y_te_target)
            std_ratio = float(np.nanstd(arr) / (denom if denom != 0 else np.nan))
            LOG.info(
                "[sanity] %s: std(yhat)/std(y)=%.3f | Spearman(score, raw)=%.3f",
                nm,
                std_ratio,
                spearman_corr(arr, y_te_raw),
            )

        # ----------------
        # Exports / reports
        # ----------------
        export_models_table(models_lin, "linear", out_dir)
        export_models_table({**models_nn, **models_trees}, "nonlinear", out_dir)
        export_linear_coefficients(models_lin, feat_names_lin, out_dir)

        # richer predictions.csv (with test-slice columns)
        aux_candidates = [
            "vega_1volpt",
            "mid_t",
        ...
        # --- Add predicted $PnL (USD) and per-dollar returns to predictions.csv
        eps = 1e-10
        if target_kind == "per_vega":
            # y_target = y_vega = dh_pnl / vega
            if "vega_1volpt" in df_te.columns:
                vega_te = df_te.loc[mask_te, "vega_1volpt"].to_numpy(dtype=float)
                for nm, yhat in preds_in_target_units.items():
                    # predicted $PnL ≈ y_vega_hat * vega
                    pred_df[f"yhat_usd_{nm}"] = yhat * vega_te

            if {"mid_t", "vega_1volpt"}.issubset(df_te.columns):
                mid_te = df_te.loc[mask_te, "mid_t"].to_numpy(dtype=float)
                denom = np.where(np.abs(mid_te) > eps, mid_te, np.nan)
                for nm in preds_in_target_units:
                    # per-dollar ≈ (y_vega_hat * vega) / mid
                    pred_df[f"yhat_per_dollar_{nm}"] = pred_df[f"yhat_usd_{nm}"] / denom

        elif target_kind == "per_dollar":
            # y_target = per-dollar return already (y_price or dh_ret)
            if "mid_t" in df_te.columns:
                mid_te = df_te.loc[mask_te, "mid_t"].to_numpy(dtype=float)
                for nm, yhat in preds_in_target_units.items():
                    # predicted $PnL ≈ per-dollar * mid
                    pred_df[f"yhat_usd_{nm}"] = yhat * mid_te
                    pred_df[f"yhat_per_dollar_{nm}"] = yhat

        elif target_kind == "per_gamma":
            # assuming y_gamma = dh_pnl / gamma_col; adapt gamma_col if needed
            gamma_col = "gamma_scaler" if "gamma_scaler" in df_te.columns else "gamma_model"
            if gamma_col in df_te.columns:
                gamma_te = df_te.loc[mask_te, gamma_col].to_numpy(dtype=float)
                for nm, yhat in preds_in_target_units.items():
                    # predicted $PnL ≈ y_gamma_hat * gamma
                    pred_df[f"yhat_usd_{nm}"] = yhat * gamma_te

                if "mid_t" in df_te.columns:
                    mid_te = df_te.loc[mask_te, "mid_t"].to_numpy(dtype=float)
                    denom = np.where(np.abs(mid_te) > eps, mid_te, np.nan)
                    for nm in preds_in_target_units:
                        # per-dollar ≈ (y_gamma_hat * gamma) / mid
                        pred_df[f"yhat_per_dollar_{nm}"] = pred_df[f"yhat_usd_{nm}"] / denom

        pred_path = os.path.join(out_dir, "predictions.csv")
        pred_df.to_csv(pred_path, index=False)
        LOG.info("Wrote predictions with aux columns: %s", pred_path)

        # Best per algorithm key for report
        ...
        preds_for_report: Dict[str, np.ndarray] = {nm: preds_in_target_units[nm] for (_, nm) in best_by_algo.values()}
        LOG.info("[report] best-per-algorithm models exported: %s", list(preds_for_report.keys()))

        export_metrics(
            y_te_target,                # target space (e.g., y_vega)
            preds_for_report,
            ids_te,
            out_dir,
            y_true_winsor_aligned=y_te_target_winsor,
            filename="metrics_target.csv",
        )

        export_diag_all(y_te_target, y_te_raw, preds_in_target_units, out_dir, ids_te)

        if cv_records:
            pd.DataFrame(cv_records).to_csv(os.path.join(out_dir, "cv_results.csv"), index=False)

        # --- Convert per-vega / per-gamma → per-dollar scores
        if target_kind == "per_vega":
            # y_target = per-vega → convert to per-dollar via price_per_vega = mid/vega
            if "price_per_vega" in df_te.columns:
                ppv_te = df_te.loc[mask_te, "price_per_vega"].to_numpy(dtype=float)
            else:
                mid = df_te.loc[mask_te, "mid_t"].to_numpy(dtype=float)
                vega = df_te.loc[mask_te, "vega_1volpt"].to_numpy(dtype=float)
                ppv_te = np.where(np.isfinite(vega) & (np.abs(vega) > eps), mid / vega, np.nan)

            finite = np.isfinite(ppv_te)
            if finite.any():
                qlo, qhi = np.nanpercentile(ppv_te[finite], [0.5, 99.5])
                ppv_use = np.clip(ppv_te, qlo, qhi)
            else:
                ppv_use = ppv_te

            ppv_use = np.where(ppv_use > eps, ppv_use, np.nan)
            # per-dollar ≈ y_vega_hat / (mid/vega) = y_vega_hat * vega / mid
            preds_scores = {name: yhat / ppv_use for name, yhat in preds_in_target_units.items()}

        elif target_kind == "per_dollar":
            # y_target already per-dollar (y_price or dh_ret)
            preds_scores = preds_in_target_units.copy()

        elif target_kind == "per_gamma":
            # assuming y_gamma = dh_pnl / gamma_col; per-dollar ≈ y_gamma * gamma / mid
            gamma_col = "gamma_scaler" if "gamma_scaler" in df_te.columns else "gamma_model"
            if (gamma_col not in df_te.columns) or ("mid_t" not in df_te.columns):
                raise ValueError("Need gamma and mid_t to convert per_gamma target to per-dollar.")
            gamma = df_te.loc[mask_te, gamma_col].to_numpy(dtype=float)
            mid = df_te.loc[mask_te, "mid_t"].to_numpy(dtype=float)
            conv = np.where(
                np.isfinite(gamma) & np.isfinite(mid) & (np.abs(mid) > eps),
                gamma / mid,
                np.nan,
            )
            finite = np.isfinite(conv)
            if finite.any():
                qlo, qhi = np.nanpercentile(conv[finite], [0.5, 99.5])
                conv = np.clip(conv, qlo, qhi)
            conv = np.where(np.abs(conv) > eps, conv, np.nan)
            preds_scores = {name: yhat * conv for name, yhat in preds_in_target_units.items()}
        else:
            # fallback: assume already per-dollar
            preds_scores = preds_in_target_units.copy()

        # Build a returns-space dict for the same best-per-algorithm set
        preds_for_report_ret = {nm: preds_scores[nm] for (_, nm) in best_by_algo.values() if nm in preds_scores}

        # Report out-of-sample R2 on per-dollar delta-hedged returns (Bali et al. comparable)
        export_metrics(
            y_te_raw,                   # per-dollar realised Δ-hedged return
            preds_for_report_ret,       # per-dollar predictions
            ids_te,
            out_dir,
            filename="metrics_returns.csv",
        )

        # quick viability diagnostics (as LOGs)
        y_ser = pd.to_numeric(pd.Series(y_te_raw), errors="coerce")
        g_ser = pd.to_datetime(pd.Series(ids_te)).dt.normalize()
        first_model = next(iter(preds_scores.values()))
        s_ser = pd.to_numeric(pd.Series(first_model), errors="coerce")
        dfc = pd.DataFrame({"g": g_ser, "y": y_ser, "s": s_ser})
        unique_days = dfc["g"].nunique()
        finite_days = dfc.dropna().groupby("g").size().shape[0]
        sizes = dfc.dropna().groupby("g").size()
        LOG.info("Unique days in test: %d", unique_days)
        LOG.info("Days with finite y & score: %d", finite_days)
        if not sizes.empty:
            LOG.info(
                "Per-day usable names: mean %.2f | min %d | max %d",
                sizes.mean(),
                sizes.min(),
                sizes.max(),
            )
            ok = (sizes >= 2 * 5)
            LOG.info("Days passing trivial min-size: %d", ok.sum())

        n_bins = int(cfg.get("portfolio", {}).get("n_bins", 10))

        export_portfolios_raw(
            y=y_te_raw,
            preds_scores=preds_scores,
            ids=ids_te,
            out_dir=os.path.join(out_dir, "port_eval"),
            n_bins=n_bins,
            opt_rel_spread=(df_te.loc[mask_te, "opt_rel_spread_final"] if "opt_rel_spread_final" in df_te else None),
            commission_bps=0.0,
            delta=(df_te.loc[mask_te, "abs_delta"] if "abs_delta" in df_te else None),
            fut_cost_bps=1.0,
            y_unit="per_dollar",
            normalize_group_to_day=True,
            min_bin_size_per_side=5,
            write_ic=True,
            verbose=True,
        )
        ...
        # SHAP (optional)
        # ----------------
        shap_cfg = (cfg.get("explain", {}) or {}).get("shap", {})
        want = shap_cfg.get("models", ["N-En"])

        all_models = {**models_lin, **models_nn, **models_trees}
        available = set(all_models.keys())

        if isinstance(want, str):
            want = [want]

        # Whether N-En SHAP was explicitly requested
        want_nen = any(str(item).strip().upper() in {"N-EN", "N_EN", "NEN"} for item in want)

        def _resolve_name(req: str) -> Optional[str]:
            if req in available:
                return req
            base = req.split("#", 1)[0]
            # pick the best-per-algorithm if present
            for b, (_, nm) in best_by_algo.items():
                if b == base:
                    return nm
            cand = [k for k in available if k.split("#", 1)[0] == base]
        ...
        for item in want:
            ...
            # N-En is handled separately
            if s_clean.upper() in {"N-EN", "N_EN", "NEN"}:
                continue
            if s_lower in {"best", "auto"}:
                targets.extend(list(preds_for_report.keys()))
            elif s_lower == "all":
                targets.extend(list(all_models.keys()))
            else:
                r = _resolve_name(s_clean)
                if r is None:
                    LOG.info(
                        "SHAP requested for '%s' but no matching model key was found; available=%s",
                        s_clean,
                        sorted(available),
                    )
                else:
        ...
        # Standard per-model SHAP
        for mname in targets:
            if mname in models_lin:
                X_te_use, feat_names_use = X_te_lin, feat_names_lin
            elif mname in models_nn:
                X_te_use, feat_names_use = X_te_ffn, feat_names_ffn
            else:
                X_te_use, feat_names_use = X_te_non, feat_names_non
            try_export_shap(all_models, X_te_use, feat_names_use, out_dir, model_name=mname, cfg=cfg)

        # Special N-En SHAP: average over GBDT, DART, RF and FFN in a common feature space
        export_shap_n_en(
        ...
        export_metrics_by_contract_bins(
            y_te_target,
            preds_for_report,
            ids_te,
            df_te_small,
        ...
        if cv_records:
            best_cv_by_model = (
                ...
                .sort_values(["model", "score_mean"], ascending=[True, False])
                .groupby("model")
                .head(1)
            )[["model", "score_mean", "params"]]
        else:
            best_cv_by_model = pd.DataFrame()

        summary.update(
            {
                "models_linear": list(models_lin.keys()),
                "models_nonlinear": list({**models_nn, **models_trees}.keys()),
                "best_cv": best_cv_by_model.to_dict(orient="records") if not best_cv_by_model.empty else [],
                "exported_best_per_algo": list(preds_for_report.keys()),
            }
        )

        with open(os.path.join(out_dir, "run_summary.json"), "w") as f:
            json.dump(summary, f, indent=2, ensure_ascii=False, default=_safe_jsonable)
        LOG.info("Variant complete.")
    finally:
...
def run_from_spec(spec: RunSpec) -> None:
    """
    Precedence (high → low): RunSpec fields > derived defaults > YAML config.
    """
    ...
    if spec.grid_sides and spec.grid_moneyness:
        for side in spec.grid_sides:
            for mny in spec.grid_moneyness:
                cfg2 = _apply_contract_overrides(cfg_base, side, mny, spec.atm_band)
                ...
                cfg2.setdefault("_run_overrides", {}).update(
                    {"label": spec.label, "groups": spec.groups, "side": side, "moneyness": mny, "atm_band": spec.atm_band, "out_dir": outdir}
                )
